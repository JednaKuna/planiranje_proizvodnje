...
def wagner2(quantity, time_period, setUp_cost, holding_cost):
    ...
    for i in range(1, time_period + 1):
        ...
        total_quantity = 0
        for j in range(i, time_period + 1):
            total_quantity += quantity[j - 1]
            if j > i:
                total_holding += (j - i) * quantity[j - 1]
            C[i - 1][j - 1] = setUp_cost + holding_cost * total_holding
    ...
    while j > 0:
        for i in range(1, j + 1):
            if F[j] == F[i - 1] + C[i - 1][j - 1]:
                production_schedule.append((i, j, sum(quantity[i - 1:j])))
                j = i - 1
                break
